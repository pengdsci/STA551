...
Filter methods are statistical-based feature selection methods that involve evaluating the relationship between each **input variable** and the **target (response) variable** using statistical relationships and selecting those input variables that have the strongest relationship with the target variable. These methods can be fast and effective, although the choice of statistical measures depends on the data type of both the input and output variables. Here are some of these methods with brief descriptions.

### Information Gain

**Information gain** calculates the reduction in entropy from the transformation of a data set. It can be used for feature selection by evaluating the Information gain of each variable in the context of the target variable. **Information Gain** answers this by measuring how much **information** a feature gives us about the target class. This amount of information is used to rank features by their ability to reduce uncertainty about the target variable. We have briefly described this method in decision induction.

**Information Gain** is NOT based on **Gini** - it's fundamentally based on **Entropy**.

Let $S$ be a dataset with $n$ classes in the target variable, the entropy of the data set is defined to be

$$
H(S) = -\sum_{i=1}^n p_i \log (p_i)
$$

$p_i$ is the proportion (probability) of instances belonging to class $i$. Assume feature variable $A$ is used as a partition variable to split the original data set into small datasets based on the distinct values of $A$. Then we can calculate the weighted sum of the entropy in the following.
...
Using the above notations, we define the information gain in the following

$$
IG(S,A) = H(S) - H(S|A).
$$

* Information Gain is natively designed for **categorical features**. The process is straightforward:
  + Split by each category: Create subsets for each unique value
  + Calculate entropy for each subset
  + Compute weighted average entropy


* For continuous features that have infinite possible values, so we can't split on each unique value. We need to **discretize them** first. The **binary splitting** is most common. The objective is to find the optimal threshold (splitting point) that maximizes information gain.
  + Sort the feature values;
  + Consider potential split points (typically midpoints between consecutive values);
  + Calculate IG for each potential split;
  + Choose the split with maximum IG.


### Chi-square Test

Let’s consider a scenario where we need to determine the relationship between the independent category feature (predictor) and dependent category feature(response). In feature selection, we aim to select the features which are highly dependent on the response. We calculate Chi-square between each feature and the response variable. and select the desired number of features with the **best (highest) Chi-square scores**. 

In order to correctly apply the chi-squared test for the relationship between various features in the data set and the target variable, the following conditions have to be met:
...
Fisher score is one of the most widely used supervised feature selection methods. It measures the **separability** between classes for each feature. It evaluates how well a feature can distinguish between different classes by looking at:

* **Between-class variance**: How far apart the class means are

* **Within-class variance**: How spread out the data is within each class

**High Fisher Score** = Feature has good separation between classes
**Low Fisher Score** = Feature has poor separation between classes

It seeks features with the best discriminant ability. To rank the features in the order of their relevancy, they are sorted in the decreasing order of their obtained fisher score. Thus, as the value of an assigned score to a feature increases, its importance also increases.
...
* For each remaining feature not in the set:
  + Temporarily add the feature to the current set
  + Train a model and evaluate performance using cross-validation
  + Record the performance metric
* Select the feature that gives the highest improvement
...
* For each feature currently in the set:
  + Temporarily remove the feature
  + Train a model and evaluate performance using cross-validation
  + Record the performance metric
  
* Remove the feature that causes the smallest performance decrease (or largest increase)
* Repeat steps 2-3 until stopping criterion is met
* Return the best feature subset found
...
* Generate all possible feature subsets of sizes 1 to n (where n is total number of features)
...
* Exhaustive Search (All Possible Subsets)
  + Evaluates all $2^n - 1$ possible non-empty subsets
  + Guaranteed to find the optimal subset
  + Computationally prohibitive for $n > 20$

* Best Subset Selection
  + Evaluates subsets of each size (1 feature, 2 features, etc.)
  + For each size k, finds the best subset of that size
  + More manageable than exhaustive search

* Genetic Algorithm-based
  + Uses evolutionary algorithms to search the feature space
...
**LASSO (Least Absolute Shrinkage and Selection Operator)** regularization is a powerful embedded method for feature selection that performs both feature selection and regularization simultaneously. It's particularly effective for dealing with high-dimensional data.

**LASSO** consists of adding a penalty to the different parameters of the machine learning model to reduce the freedom of the model, i.e. to avoid over-fitting. In linear model regularization, the penalty is applied over the coefficients that multiply each of the predictors. From the different types of regularization, Lasso or L1 has the property that can shrink some of the coefficients to zero. Therefore, that feature can be removed from the model.


For a standard linear regression model

$$
\min_{\beta}\left(\frac{1}{2n}\sum_{i=1}^n \left[ y_i - \beta_0-\sum_{k=1}^p \beta_k x_k\right]^2\right)
$$

The corresponding LASSO linear regression is

$$
\min_{\beta}\left(\frac{1}{2n}\sum_{i=1}^n \left[ y_i - \beta_0-\sum_{k=1}^p \beta_k x_k\right]^2 + \lambda\sum_{j=1}^p |\beta_j|\right)
$$

The solution to the optimization problems based on the above two models are illustrated in the following figures.


```{r echo=FALSE, fig.align='center',out.width="95%", fig.cap="Figure 0. LASSO Linear Regression."}
include_graphics("img/lasso.png")
```
...
* Mean Decrease in Impurity (MDI/Gini Importance)
  + Based on how much each feature decreases impurity (Gini/Entropy) across all trees
  + Calculated during training

* Permutation Importance
  + Based on how much permuting a feature decreases model performance
  + Computed after training
  + More reliable but computationally expensive


**Advantages and Limitations of Random Forest Importance**

* Advantages 
  + *Handles non-linear relationships*: Captures complex feature interactions
  + *Robust to outliers*: Less sensitive to extreme values
  + *No distribution assumptions*: Works with various data types
  + *Provides reliable rankings*: Generally stable across different samples
  + *Interpretable results*: Easy to understand and explain

* Limitations
  + *Biased towards high-cardinality features*: May favor features with more categories
  + *Correlated features*: Importance can be spread among correlated features
  + *Computational cost*: Can be slow with large datasets and many trees
  + *Randomness*: Results can vary with different random seeds
